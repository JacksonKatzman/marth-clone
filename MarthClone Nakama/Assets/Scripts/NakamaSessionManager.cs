...
namespace DemoGame.Scripts.Session
{
    ...
    public class NakamaSessionManager : Singleton<NakamaSessionManager>
    {
        ...
        private void Start()
        {
            ...
            GetDeviceId();
            //await ConnectAsync();
            //GameManager.instance.AttemptConnect();
        }
        ...
        private async Task<AuthenticationResponse> RestoreTokenAsync()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        // Account not found
                        Debug.Log("Acct not found even though session exists.");
        ...
        {
            Debug.Log("Started GetDeviceID");
            if (string.IsNullOrEmpty(_deviceId) == true)
            {
                _deviceId = PlayerPrefs.GetString("nakama.deviceId");
                Debug.Log("Was empty, looked at player prefs, set to: " + _deviceId);
                if (string.IsNullOrWhiteSpace(_deviceId) == true)
                {
                    _deviceId = SystemInfo.deviceUniqueIdentifier;
                    Debug.Log("Was sTILL empty, created a new one, set to: " + _deviceId);
